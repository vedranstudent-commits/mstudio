...
export default function ClientCreate(){
  ...
  const [name,setName]=useState(''); const [email,setEmail]=useState(''); const [phone,setPhone]=useState(''); const [notes,setNotes]=useState(''); const [saving,setSaving]=useState(false);
  async function save(){ setSaving(true); await supabase.from('clients').insert({ name, contact:{email,phone}, notes }); setSaving(false); setName(''); setEmail(''); setPhone(''); setNotes(''); }
  return (<><button onClick={()=>setOpen(true)} className="btn">+ Novi klijent</button>
