...
export default function HomePage(){
  const [metrics,setMetrics]=useState({tasks:0});
  useEffect(()=>{(async()=>{const t=await supabase.from('tasks').select('id',{count:'exact',head:true}); setMetrics(m=>({...m,tasks:t.count||0}));})()},[]);
